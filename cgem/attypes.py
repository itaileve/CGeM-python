...
def classify_nitrogen(atoms, coords_c):
    ...
    for i, atom in enumerate(atoms):
        if atom == 7:
            ...
            neighbors = np.where(distance_vec < 1.4)[1]
            n_H_neighbor = 0
            for j in neighbors:
...
def classify_charge_pdb(atoms, coords_c, type_pdb, amacid_type):
    """Do atom typing for nitrogen atoms.
    ...
    for i, atom in enumerate(atoms):
        ...
        if type_pdb[i]=="NZ" and amacid_type[i]=="LYS": # N+ Lysine
            print(f"Positive nitrogen at : {i}")
            classified_atoms[i] = 7001
            predicted_charge += 1
            # change atom types for directly bonded H and C to 110 and 610
        if type_pdb[i]=="HZ1" and amacid_type[i]=="LYS":# H Bonded to N+ LYS
            classified_atoms[i] = 110
        if type_pdb[i]=="HZ2" and amacid_type[i]=="LYS":# H Bonded to N+ LYS
            classified_atoms[i] = 110
        if type_pdb[i]=="HZ3" and amacid_type[i]=="LYS":# H Bonded to N+ LYS
            classified_atoms[i] = 110
        if type_pdb[i]=="CE" and amacid_type[i]=="LYS":# C Bonded to N+ LYS
            classified_atoms[i] = 610
            
        if type_pdb[i]=="OE2" and amacid_type[i]=="GLU": # C- in GLU
            coords_s = add_shell(coords_s, i)
            print(f"Negative oxygen at : {i}")
            predicted_charge -= 1
            classified_atoms[i] = 820
            # ARG carbon center
        if type_pdb[i]=="OD2" and amacid_type[i]=="ASP": # C- in ASP
            coords_s = add_shell(coords_s, i)
            print(f"Negative oxygen at : {i}")
            predicted_charge -= 1
            classified_atoms[i] = 820
            # ARG carbon center
        if type_pdb[i]=="CZ" and amacid_type[i]=="ARG": # carbon Bonded to N+ ARG
            print(f"Positive carbon at : {i}")
            classified_atoms[i] = 6001
            predicted_charge += 1
        if type_pdb[i]=="1HH1" and amacid_type[i]=="ARG": # H Bonded to N+ ARG 
            classified_atoms[i] = 110
        if type_pdb[i]=="2HH1" and amacid_type[i]=="ARG": # H Bonded to N+ ARG 
            classified_atoms[i] = 110
        if type_pdb[i]=="1HH2" and amacid_type[i]=="ARG": # H Bonded to N+ ARG 
            classified_atoms[i] = 110
        if type_pdb[i]=="2HH2" and amacid_type[i]=="ARG": # H Bonded to N+ ARG 
            classified_atoms[i] = 110
