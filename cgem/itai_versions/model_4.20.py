...
class CGem(object):
    def __init__(
        ...
        self._r_cut = r_cut
        self._longterm_cutoff = longterm_cutoff
        ...
        if opt_shells:
            coords_s = self._optimize_shell_coordinates(coords_s, method=method)
    ...
    def from_molecule(
        ...
        **kwargs,
        ...
        method: str
            optimization method that goes into the scipy.optimize.minimize function
        r_cut: float/int/None, optional
            cutoff radius in Angstrom for tapering function. Defualt is None (don't use tapering).
            In Itai's water model cuttoff is set to 10 angstrom.
        longterm_cutoff: float/int/None, optional
            cutoff radius in Angstrom for tapering function 2. Defualt is None (don't use tapering).
            When the shells are optimized with full energy function but the in reported energy and forces only
                long ranged components are kept. For ML-FF project longterm_cutoff is 5.
        kwargs : optional
    ...
    @property
    def longterm_cutoff(self):
        """ long range cutoff radius in Angstrom for tapering function 2. Defualt is None (don't use tapering)"""
        return self._longterm_cutoff

    @property
    def energy(self):
        """Total interaction energy."""
        return self.compute_energy(self.coords_s,longterm_cutoff=self.longterm_cutoff)

    @property
    def force(self):
        """Force with respect to change of core positions"""
        # TODO
        return self.compute_force(self.coords_s,longterm_cutoff=self.longterm_cutoff)
    ...
    def compute_coulomb_terms(self, coords_s, deriv=False, longterm_cutoff=None):
        ...
        ss = compute_coulomb_ss(
            coords_s, self.charge_s, self.alpha_s, deriv=deriv, r_cut=self.r_cut, longterm_cutoff=longterm_cutoff,
        )
        cs = compute_coulomb_cs(
            self.coords_c,
            self.charge_c,
            self.alpha_c,
            coords_s,
            ...
            deriv=deriv,
            r_cut=self.r_cut,
            longterm_cutoff = longterm_cutoff,
        )
        cc = compute_coulomb_cc(self.coords_c,self.charge_c,self.alpha_c,r_cut=self.r_cut, longterm_cutoff=longterm_cutoff)
        if deriv:
    ...
    def compute_coulomb_forces(self, coords_s, longterm_cutoff=None):
        if len(coords_s) == 0:  # no shell
            return 0
        # compute shell-shell & core-shell Coulomb interactions
        # shape(n,3) n is number of core, m is number of shell
        cc = compute_coulomb_cc_force(
            self.coords_c, self.charge_c, self.alpha_c, r_cut=self.r_cut, longterm_cutoff=longterm_cutoff
        )
        # shape (n,3)
    ...
    def compute_gaussian_terms(self, coords_s, deriv=False, longterm_cutoff=None,):
    ...
    def compute_gaussian_forces(self, coords_s, longterm_cutoff=None):
        ...
        if len(self.coords_c) == 0:
            ...
            d_cs = compute_gaussian_cs_force(
                self.coords_c, self.gamma_c, self.beta_c, coords_s, r_cut=self.r_cut,longterm_cutoff=longterm_cutoff,
            )

        return d_cs

    def compute_energy(self, coords_s, deriv=False, longterm_cutoff=None):
        ...
        term_c = self.compute_coulomb_terms(coords_s, deriv=deriv, longterm_cutoff=longterm_cutoff)
        term_g = self.compute_gaussian_terms(coords_s, deriv=deriv, longterm_cutoff=longterm_cutoff)
        if deriv:
            # computed values are tuples of value & its derivative
            return term_c[0] + term_g[0], term_c[1] + term_g[1]
        return term_c + term_g

    def compute_force(self, coords_s, longterm_cutoff=None):
        ...
        if len(coords_s) != len(self.charge_s):
            raise ValueError(f"Expected {len(self.charge_s)} shell coordinates.")
        # compute Coulomb & Gaussian terms of energy
        term_c = self.compute_coulomb_forces(coords_s, longterm_cutoff=longterm_cutoff)
        term_g = self.compute_gaussian_forces(coords_s, longterm_cutoff=longterm_cutoff)

        return -(term_c + term_g)

    def _optimize_shell_coordinates(self, guess, method="L-BFGS-B"):
        """Optimize shell coordinates.
    ...
    def compute_electrostatic_potential_gaussian_charge(self, points):
        ...
        value = compute_pairwise_coulomb(
            self.coords_c, self.charge_c, self.alpha_c, points, charge, alphas
        )
        # electrostatic potential of shells
        shell_value = compute_pairwise_coulomb(
            self.coords_s, self.charge_s, self.alpha_s, points, charge, alphas
        )
...
def compute_pairwise_coulomb(
    ...
    if deriv:
        ...
        if r_cut is not None:
            ...
            value = value * tap(dist,  r_cut)
        if longterm_cutoff is not None:
            longterm_cutoff = np.full(dist.shape, longterm_cutoff)
            d_value = d_tap2(dist, longterm_cutoff) * value + tap2(dist, longterm_cutoff) * d_value
            value = value * tap2(dist, longterm_cutoff)
        d_value /= dist
    ...
    if longterm_cutoff is not None:
        value = value * tap2(dist, np.full(dist.shape, longterm_cutoff))
    return value


def compute_pairwise_coulomb_force(
    coord1, charge1, alpha1, coord2, charge2, alpha2, threshold=1.0e-8, r_cut=None,longterm_cutoff=None
):
    ...
    if r_cut is not None:
        r_cut = np.full(dist.shape, r_cut)
        d_value = d_tap(dist, r_cut) * value + tap(dist, r_cut) * d_value
        value = value * tap(dist, r_cut) #np.full(dist.shape, r_cut))
    if longterm_cutoff is not None:
        longterm_cutoff = np.full(dist.shape, longterm_cutoff)
        d_value = d_tap2(dist, longterm_cutoff) * value + tap2(dist, longterm_cutoff) * d_value
        value = value * tap2(dist, longterm_cutoff)
...
def compute_gaussian_cs(coords_c, gamma_c, beta_c, coords_s, deriv=False, r_cut=None,longterm_cutoff=None):
    ...
    if deriv:
        ...
        if r_cut is not None:
            ...
            dist_tmp[dist_tmp == 0] = 1e-10
            r_cut = np.full(dist.shape, r_cut)
            d_value = d_tap(dist, r_cut) * value / dist_tmp + tap(dist, r_cut) * d_value
            value = value * tap(dist, r_cut) #np.full(dist.shape, r_cut))
        if longterm_cutoff is not None:
            dist_tmp = np.copy(dist)
            dist_tmp[dist_tmp==0] = 1e-10
            longterm_cutoff = np.full(dist.shape, longterm_cutoff)
            d_value = d_tap2(dist, longterm_cutoff) * value / dist_tmp + tap2(dist, longterm_cutoff) * d_value
            value = value * tap2(dist, longterm_cutoff)
        # convert d_value wrt r to x,y,z component
...
def compute_gaussian_cs_force(coords_c, gamma_c, beta_c, coords_s, r_cut=None, longterm_cutoff = None):
    dist = cdist(coords_c, coords_s)
    value = np.exp(-(dist ** 2) * gamma_c[:, np.newaxis])
    value = value * beta_c[:, np.newaxis]
...
def compute_gaussian_ss(
    coords, gamma, beta, deriv=False, penalty_param=(10, 200), r_cut=None, longterm_cutoff = None
):
...
def compute_coulomb_cc(coords, charges, alphas, r_cut=None, longterm_cutoff = None):
    if len(coords) <= 1:
        cc = 0
    else:
        cc = compute_pairwise_coulomb(
            coords, charges, alphas, coords, charges, alphas, r_cut=r_cut, longterm_cutoff = longterm_cutoff
        )
        cc = np.sum(cc[np.triu_indices(len(cc), k=1)])
    return cc


def compute_coulomb_cc_force(coords, charges, alphas, r_cut=None, longterm_cutoff = None):
    if len(coords) <= 1:
        d_cc = 0
    else:
...
def compute_coulomb_ss(coords, charges, alphas, deriv=False, r_cut=None, longterm_cutoff=None):
    if len(coords) <= 1:
        if deriv:
            return 0, 0
        else:
    ...
    ss, d_ss = compute_pairwise_coulomb(
        coords, charges, alphas, coords, charges, alphas, deriv=True, r_cut=r_cut, longterm_cutoff=longterm_cutoff
    )
...
def compute_coulomb_cs_force(
    coords_c, charges_c, alphas_c, coords_s, charges_s, alphas_s, r_cut=None, longterm_cutoff=None
):
    if len(coords_c) < 1 or len(coords_s) < 1:
        d_cs = 0, 0
    else:
        d_cs = compute_pairwise_coulomb_force(
            coords_c, charges_c, alphas_c, coords_s, charges_s, alphas_s, r_cut=r_cut, longterm_cutoff=longterm_cutoff
        )
...
def tap(r, r_cut):
    ...
    This tapering function comes from I. Leven, H. Hao, A. K. Das, T. Head-Gordon, J. Phys. Chem. Lett. 2020, 11, 9240.,
...
def tap2(r, r_cut):
    """
    a tappering function that keep the r>r_cut intact while muting the short range
    r,r_cut are np.ndarray of same shape
    """
    return 1-tap(r,r_cut)


def d_tap2(r, r_cut):
    return  -d_tap(r,r_cut)
